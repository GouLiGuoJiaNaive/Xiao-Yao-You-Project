...
-- AUTHORITY
	...
	self:add_ceo_unlock_listener("3k_dlc04_ceo_factional_great_library_authority_refined",
		"FactionTurnStart",
		function(context)
			if not self:is_lz_faction(context:faction()) then
				return false;
			end;

			if not context:faction():has_faction_leader() then
				return false;
			end;

			local query_leader = context:faction():faction_leader();
			if not query_leader:has_region() then
				return false;
			end

			local leader_region = query_leader:region():name();
			return leader_region == "3k_main_anding_capital" or leader_region == "3k_main_anding_resource_1" or leader_region == "3k_main_anding_resource_2" or leader_region == "3k_main_anding_resource_3"
		end
	);
