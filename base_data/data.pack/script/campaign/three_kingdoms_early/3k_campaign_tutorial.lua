...
force_require("lib_state_machine");
...
--checks to see if the faction key passed has an active tutorial or not
function campaign_tutorial:faction_has_tutorial(faction_key)

	if not is_string(faction_key) then
		script_error("Campaign Tutorial Error - faction key is not string in faction_has_tutorial function")
	end

	if not cm:query_faction(faction_key) or cm:query_faction(faction_key):is_null_interface() then
		script_error("Campaign Tutorial Error - invalid faction key passed")
	end

	if self.faction_settings[faction_key] then
		return true
	else
		return false
	end
	...
	sm:add_state( states.ATTACK_ENEMY_TOWN, -- name
		function(context)
			...
			cm:wait_for_model_sp(function() 
				...
				if not region then
					script_error("ERROR: scroll_camera_with_cutscene_to_settlement() called but region with supplied key [" .. tostring(self.target_settlement_key) .. "] could not be found");
					return false;
				end;
			...
			cm:wait_for_model_sp(function() 		
				cm:callback(
					function()				
						...
						if not region then
							script_error("ERROR: scroll_camera_with_cutscene_to_settlement() called but region with supplied key [" .. tostring(self.own_settlement_key) .. "] could not be found");
							return false;
						end;
