...
cdir_mission_manager:start_incident_db_listener(
	...
	"3k_dlc07_introduction_liu_yan_incident_190", -- event_key 
	...
	success_key.."intro_fired" -- failure event
);
...
cdir_mission_manager:start_mission_listener(
	...
	"3k_dlc07_progression_liu_yan_190_destroy_faction",                     -- mission key
	"DESTROY_FACTION",                                  -- objective type
	{
		"faction 3k_main_faction_jia_long"
	},   
	{
		"money 5000"
	...
	false,												-- Fire once
	success_key.."jia_long_attacked",		-- completion event
	success_key.."jia_long_attacked",		-- failure event
	"CLAN_ELDERS"							--mission_issuer
);

cdir_mission_manager:start_mission_db_listener(
	local_faction_key, -- faction_key 
	"3k_dlc07_progression_liu_yan_190_perform_assignment", -- event_key 
	success_key.."jia_long_attacked", -- trigger event 
	function(context)
		return true;
	end, --listener condition
	false, -- fire_once.
	success_key.."assignment_started", -- completion event 
	failure_key.."assignment_started", -- failure event
	false -- delay start
);
...
cdir_mission_manager:start_mission_db_listener(
	local_faction_key, -- faction_key 
	"3k_dlc07_progression_liu_yan_190_construct_building", -- event_key 
	success_key.."assignment_started", -- trigger event 
	function(context)
		return true;
	end, --listener condition
	false, -- fire_once.
...
cdir_mission_manager:start_mission_db_listener(
	local_faction_key, -- faction_key 
	"3k_dlc07_progression_liu_yan_190_balance_economy", -- event_key 
	success_key.."building_constructed", -- trigger event 
	function(context)
		return true;
	end, --listener condition
	false, -- fire_once.
	success_key.."economy_balanced", -- completion event 
	failure_key.."economy_balanced", -- failure event
	false -- delay start
);
...
cdir_mission_manager:start_mission_listener(
	...
	success_key.."economy_balanced",      -- trigger event 
	nil,												-- Listener condition
	false,												-- Fire once
...
-----------------------------------HELPERS---------------------------------------
core:add_listener(
	listener_key.."aid_against_DZ_fxb_canceller",
	"FactionTurnStart",
	function(context)
		return context:faction():name() == local_faction_key and context:faction():is_human()
		 and diplomacy_manager:is_at_war_with(local_faction_key, "3k_main_faction_ma_teng")
		 and context:faction():has_effect_bundle("3k_dlc07_effect_bundle_liu_yan_aid_against_dong_zhuo")
	end,
	function(context)
		cm:modify_faction(local_faction_key):remove_effect_bundle("3k_dlc07_effect_bundle_liu_yan_aid_against_dong_zhuo")
		core:remove_listener(listener_key.."aid_against_DZ_fxb_canceller")
	end,
	true
)