...
function ep_emperor_manager:initialise()
  ...
  force_require("ep_storybook") -- the storybook already saves if the player is Emperor or not, so we can use that info here.
...
---if the emperor changes hands, decided what to do. If the player has captured him in battle, ask whether they're Emperor - if so, give them the Emperor Captured dilemma. If not, update the UI to show that they're now Regent and kill the Empress.
  core:add_listener(
	...
	function(context)
		-- If the attacker isn't present or available then return false as it's required by the function below.
		if not context:query_model():pending_battle():attacker() or context:query_model():pending_battle():attacker():is_null_interface() then
			return false;
		end;
