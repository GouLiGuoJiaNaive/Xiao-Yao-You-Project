...
output("3k_dlc07_gating.lua: Loading");
...
function setup_gating()
	...
	local function has_unlocked_tax_slider(query_faction)
		return progression:has_progression_feature(query_faction, "tax_slider", false); -- DLC07 -- Replaced progression level test with feature_test
	end;
	...
	local function has_unlocked_faction_council( query_faction )
		return query_faction:is_human() and progression:has_progression_feature(query_faction, "faction_council", false)  -- DLC07 -- Replaced progression level test with feature_test;
	end;
    ...
    if not uim:override("tax_slider"):get_allowed() then
        ...
        core:add_listener(
            ...
            "FactionFameLevelUp", 
			function(context)
				return context:faction():is_human() and has_unlocked_tax_slider( context:faction() );
            end,
            function(context)
