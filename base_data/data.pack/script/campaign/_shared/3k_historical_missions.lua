---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
----- Name:			3k_historical_missions.lua
----- Description: 	Three Kingdoms system to handle historical missions. Deprecated by 3k_campaign_cdir_mission_manager.lua
---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------

out("3k_historical_missions.lua: Loading");

function start_historical_mission_listener(faction_key, mission_key, objective, conditions, mission_rewards, trigger_event, completion_event, precondition, failure_event, optional_mission_issuer, optional_turn_limit)
	optional_mission_issuer = optional_mission_issuer or "SHOGUN";
	optional_turn_limit = optional_turn_limit or 0;
	...
	if optional_turn_limit and not is_number(optional_turn_limit)then 
		script_error("ERROR: start_historical_mission_listener() called but supplied mission turn limit [" .. tostring(optional_turn_limit) .. "] is not a number or nil")
		return false
	 end
    ...
    local mm = mission_manager:new(
		...
		nil,	--nearing expiry callback. we don't use this
		optional_turn_limit
    );
            ...
            function()
                if not precondition or precondition() then
                    ...
                    if objective == "OWN_N_UNITS" then
                        mm:add_condition("total " .. get_player_number_of_units(faction_key) + conditions[1])
                    end;
                    ...
                    if objective == "ENGAGE_FORCE" then
                        mm:add_condition("cqi " .. get_cqi_of_nearest_enemy(faction_key, conditions[1]))
					end;
...
function get_cqi_of_nearest_enemy(faction_key, enemy_faction)
...
function get_player_number_of_units(faction_key)
...
function start_historical_mission_cancel_listener(faction_key, mission_keys, failure_event)
    ...
    core:add_listener(
        ...
        "FactionFameLevelUp",
        function(context)
			return context:faction():name() == faction_key and progression:has_progression_feature(context:faction(), "rank_second_marquis");
        end,
		function(context)
			-- Remove all listeners first or the cancelled missions will cascade fire/abort themselves.
			for i = 1, #mission_keys do
			...
			for i = 1, #mission_keys do
				...
				-- Always fire the failure event in-case something outside of the system is waiting for it.
