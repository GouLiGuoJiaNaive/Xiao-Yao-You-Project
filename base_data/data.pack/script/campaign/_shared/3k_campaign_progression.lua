...
function progression:force_campaign_defeat(faction_reference)
	...
	if cm:is_multiplayer() then
		if diplomacy_manager:is_mp_coop_victory_enabled(query_faction) then
			--cm:modify_faction(query_faction):cancel_custom_mission("3k_main_mp_coop_victory");
		else
			--cm:modify_faction(query_faction):cancel_custom_mission("3k_main_mp_versus_victory");
		end;
	else
