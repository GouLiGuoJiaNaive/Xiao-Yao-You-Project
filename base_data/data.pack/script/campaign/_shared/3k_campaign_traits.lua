...
cm:add_first_tick_callback_new(function() traits:new_game() end); -- Selt register new game
cm:add_first_tick_callback(function() traits:initialise() end); --Self register function
...
force_require("3k_campaign_traits_data");

---------------------------------------------------------------------------------------------------------
----- MAIN FUNCTIONS
---------------------------------------------------------------------------------------------------------

function traits:new_game()
	output("3k_campaign_traits.lua: New Game()");

	self:setup_new_game_legendary_traits();
end;

--// initialise()
--// Entry point.
function traits:initialise()
    output("3k_campaign_traits.lua: Initialise()" );
...
function traits:add_protagonist_points(modify_character, xp_gained)
	...
	self:print("3k_campaign_traits.lua: add_protagonist_points(): Adding: " .. tostring(points) .. " points.", true);
...
function traits:fire_character_ceo_trigger(modify_character, trigger_key)
	...
	if self.debug_mode then 
		self:print("Firing CEO Trigger - " .. tostring(modify_character:query_character():command_queue_index()) .. ": ".. tostring(trigger_key) .. " - " .. tostring(modify_character:query_character():generation_template_key()), true );
	end;
...
function traits:fire_from_pending_trigger_data_old( modify_character, use_element )
	...
	local weighted_trigger_datas = table.copy(self.pending_trigger_datas); -- Create a local copy as we'll manipulate this.
...
function traits:validate_trigger_datas( trigger_data_list )
	...
	if table.length(trigger_data_list) < 1 then
...
function traits:validate_trigger( trigger_data )
	...
	if not is_number(trigger_data.priority) then
		script_error( "validate_trigger_datas() badly formed trigger, PRIORITY should be a number." .. tostring(trigger_data.priority) );
		is_valid = false;
	end;
	
	if not is_table(trigger_data.triggers) then
		script_error( "validate_trigger_datas() badly formed trigger, TRIGGERS should be a table." .. tostring(trigger_data.triggers) );
		is_valid = false;
	end;

	if is_table(trigger_data.triggers) then
		if #trigger_data.triggers == 0 then
			script_error( "validate_trigger_datas() badly formed trigger, TRIGGERS has 0 elements." );
			is_valid = false;
		end;

		for i, trigger in ipairs( trigger_data.triggers ) do
			if not is_string(trigger.trigger_key) then
				script_error( "validate_trigger_datas() badly formed trigger, child trigger data 'trigger_key' must be a string." .. tostring(trigger.trigger_key) );
				is_valid = false;
			end;
		
			if not is_string(trigger.element) then
				script_error( "validate_trigger_datas() badly formed trigger, child trigger data 'element' must be a string." .. tostring(trigger.element)  );
				is_valid = false;
			end;
		
			if not is_number(trigger.num_ceos) then
				script_error( "validate_trigger_datas() badly formed trigger, child trigger data 'num_ceos' must be a number." .. tostring(trigger.num_ceos)  );
				is_valid = false;
			end;
		end;
	end;
...
function traits:print(string_to_print, debug_only)
	if not self.debug_mode and debug_only then
		return false;
	end;

	out.traits("[TRTS] " .. tostring(string_to_print));
end;
