---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
----- Name:			3k_campaign_invasions.lua, 
----- Description: 	Three Kingdoms system for scripted invasions
---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
...
campaign_invasions.unit_group_default = {amount_group = "normal", unit_list = "3k_main_chinese", commander_list = "3k_main_chinese"};
...
function campaign_invasions:create_invasion(invasion_faction_key, spawn_region, difficulty_rating, opt_delay_start, opt_target_faction, opt_suppress_event_feed, opt_x, opt_y)
	...
	if not is_string(invasion_faction_key) then
		script_error("ERROR: create_invasion() called but supplied faction value [" .. tostring(invasion_faction_key) .. "] is not nil or a boolean");
		return false;
	end;

	if not is_string(spawn_region) then
		script_error("ERROR: create_invasion() called but supplied region value [" .. tostring(spawn_region) .. "] is not nil or a boolean");
		return false;
	end;

	if not is_number(difficulty_rating) then
		script_error("ERROR: create_invasion() called but supplied difficulty value [" .. tostring(difficulty_rating) .. "] is not nil or a boolean");
		return false;
	end;

	if opt_delay_start and not is_boolean(opt_delay_start) then
		script_error("ERROR: create_invasion() called but supplied delay_start value [" .. tostring(opt_delay_start) .. "] is not nil or a boolean");
		return false;
	end;

	if opt_target_faction and not is_string(opt_target_faction) then
		script_error("ERROR: create_invasion() called but supplied target faction value [" .. tostring(opt_target_faction) .. "] is not nil or a boolean");
		return false;
	end;

	if opt_suppress_event_feed and not is_boolean(opt_suppress_event_feed) then
		script_error("ERROR: create_invasion() called but supplied opt_suppress_event_feed value [" .. tostring(opt_suppress_event_feed) .. "] is not nil or a boolean");
		return false;
	end;

	if opt_x and not is_number(opt_x) then
		script_error("ERROR: create_invasion() called but supplied opt_x value [" .. tostring(opt_x) .. "] is not nil or a boolean");
		return false;
	end;

	if opt_y and not is_number(opt_y) then
		script_error("ERROR: create_invasion() called but supplied opt_y value [" .. tostring(opt_y) .. "] is not nil or a boolean");
		return false;
	end;
	...
	if not invasion_query_faction or invasion_query_faction:is_null_interface() then
		script_error("ERROR: create_invasion() called but cannot find an invasion faction for key [" .. tostring(invasion_faction_key) .. "]");
		return false;
	end;
	...
	if not unit_group then
		script_error("Force Generation Failed: Valid unit group.");
		return false;
	end;
	...
	if not general_subtype then
		script_error("Force Generation Failed: No General");
		return false;
	end;
	...
	if not num_units then
		script_error("Force Generation Failed: Unable to find unit amount.");
		return false;
	end;
	...
	if not found_spawn then
		script_error("Force Generation Failed: No valid spawn location found");
		return false;
	end;
...
function campaign_invasions:create_invasion_attack_faction(invasion_faction_key, spawn_region, difficulty_rating, target_faction_key, opt_suppress_event_feed, opt_x, opt_y)
	...
	invasion:set_target("NONE", nil, target_faction_key);
...
function campaign_invasions:impl_get_best_unit_group(faction_key, campaign_key, culture_key, subculture_key)
	...
	script_error("Error unable to find group match on the following criteria. [faction: " .. tostring(faction_key) .. "] [campaign: " .. tostring(campaign_key) .. "] [subculture: " .. tostring(subculture_key) .. "] [culture_key: " .. tostring(culture_key) .. "]");
	return nil;
end;
