---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
----- Name:			3k_ytr_campaign_traits.lua, 
----- Description: 	Three Kingdoms system to hangle YT learning traits.
---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------

-- Early exit if we're in eight princes.
if cm.name == "ep_eight_princes" then
	output("yt_traits: Not loaded in this campaign." );
	return;
end;

cm:add_first_tick_callback(function() yt_traits:initialise() end); --Self register function
...
function yt_traits:turn_start_learning_growth( modify_character )
	...
	if not equipped_learning_trait or equipped_learning_trait:is_null_interface() then
		script_error("ERROR: turn_start_learning_growth() No Learning trait found! faction[" .. modify_character:query_character():faction():name() .. "] character[" .. modify_character:query_character():generation_template_key() .. "]");
		return false;
	end;
...
function yt_traits:convert_teacher_to_emperor( query_character, modify_character )
	...
	if not yt_emperor_ascension or not yt_emperor_ascension:has_ascended( query_character:faction() ) then
		return false;
	end;
