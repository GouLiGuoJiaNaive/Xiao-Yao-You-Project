---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
----- Name:			3k_campaign_character_relationships.lua
----- Description: 	Three Kingdoms system to fire bespoke relationship triggers for characters.
---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------

cm:add_first_tick_callback(function() character_relationships:initialise() end); --Self register function
...
function character_relationships:character_spawn_remove_from_targets(query_character)
	...
	local ind = self:character_spawn_get_target_index(character_cqi);
	if ind and ind > 0 then -- only remove if we found them.
		table.remove( self.character_spawn_relationship_targets, ind );
	end;
end;
...
function character_relationships:character_spawn_get_target_index(cqi)
	...
	for i=1, #self.character_spawn_relationship_targets do
		if self.character_spawn_relationship_targets[i] == cqi then
			index = i;
			break;
		end;
	end;
