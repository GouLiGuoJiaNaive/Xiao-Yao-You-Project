...
cao_cao_schemes = {
	...
	schemes_data = {
		["3k_dlc07_scheme_army_attacking_when_far_appear_near"] = {
			...
			precondition = "RegionOwnerFactionContext == PlayersFaction || CaoCaoHasMilitaryForceInProvince( this.ClosestSettlementContext.ProvinceContext )",
		...
		["3k_dlc07_scheme_army_campaigning_hawk_and_tiger_manoeuvres"] = {
			...
			target = "self",
			attributes_key = "cunning", 
			precondition = "CommandingCharacterContext.ActionPointsPerTurn > 0 && CommandingCharacterContext.ActionPointPercent < 0.1",
			--Info for script use only, UI shouldn't need this
		...
		["3k_dlc07_scheme_army_sieging_release_the_damned_river"] = {
			...
			target = "enemy",
			attributes_key = "cunning", 
			precondition = "SettlementContext.IsUnderSiege && SettlementContext.BesiegingForceContext.FactionContext == PlayersFaction && SettlementContext.BesiegingForceContext.BesiegingForceBalance <= 0.33",
			--Info for script use only, UI shouldn't need this
			effect_bundle_duration = 5,
			turns_till_active = 0
		},
		...
		["3k_dlc07_scheme_character_low_satisfaction_eyes_and_ears_within"] = {
			...
			target = "enemy",
			attributes_key = "cunning", 
			precondition = "SatisfactionValue <= 25",
			--Info for script use only, UI shouldn't need this
			effect_bundle_duration = 5,
			turns_till_active = 0
		},
		["3k_dlc07_scheme_character_low_satisfaction_knowledge_of_the_ambitious"] = {
			...
			target = "self",
			attributes_key = "cunning", 
			precondition = "SatisfactionValue <= 10 && NegativeSatisfactionModifierList.FindIf( RecordContext.Key == 'job_satisfaction' ).Index != -1",
			--Info for script use only, UI shouldn't need this
			effect_bundle_duration = 5,
			turns_till_active = 0
		},
		...
		["3k_dlc07_scheme_province_faction_support_douse_the_fires_of_rebellion"] = {
			...
			target = "enemy",
			attributes_key = "cunning", 
			precondition = "CaoCaoHasMilitaryForceInProvince( this ) && SettlementList.Any( (x) => x.IsPlayerOwned == false && x.FactionSupportContext.OwningFactionSupportDataContext.PercentageValue >= 0.5 )",
			--Info for script use only, UI shouldn't need this
			effect_bundle_duration = 5,
			turns_till_active = 0
		},
		["3k_dlc07_scheme_province_low_population_contain_the_flood_not_the_river"] = {
			...
			target = "enemy",
			attributes_key = "cunning", 
			precondition = "IsPlayerCompleteOwner == false && CaoCaoHasMilitaryForceInProvince( this ) && InvolvedFactionsList.Filter( IsRebelFaction == false ).Any( (f) => this.PooledResourceViewContext('3k_main_pooled_resource_population', f).PercentageOfCapacity > 50 )",
			--Info for script use only, UI shouldn't need this
			effect_bundle_duration = 5,
			turns_till_active = 0
		},
		["3k_dlc07_scheme_province_low_public_order_foresight_when_blind"] = {
			...
			target = "enemy",
			attributes_key = "cunning", 
			precondition = "IsPlayerCompleteOwner == false && CaoCaoHasMilitaryForceInProvince( this ) && InvolvedFactionsList.Filter( IsRebelFaction == false ).Any( (x) => this.PublicOrderContext(x).PublicOrder < 50 )",
			--Info for script use only, UI shouldn't need this
			effect_bundle_duration = 5,
			turns_till_active = 0
		},
		...
		["3k_dlc07_scheme_region_construction_the_wolf_guarding_over_the_chickens"] = {
			...
			target = "enemy",
			attributes_key = "cunning", 
			precondition = "IsPlayerCompleteOwner == false && CaoCaoHasMilitaryForceInProvince( this ) && InvolvedFactionsList.Filter( IsRebelFaction == false ).Any( (f) => this.PooledResourceContext('3k_main_pooled_resource_supply', f).Total > 20 )",
			--Info for script use only, UI shouldn't need this
			effect_bundle_duration = 5,
			turns_till_active = 0
		},
		...
		["3k_dlc07_scheme_region_neutral_presence_children_in_the_hand"] = {
			...
			target = "enemy",
			attributes_key = "cunning", 
			precondition = "IsContextValid( DiplomacyAllianceContext ) == false || DiplomacyAllianceContext.FactionList.Find( PlayersFaction ).Index == -1",
			--Info for script use only, UI shouldn't need this
			effect_bundle_duration = 1,
			turns_till_active = 0
		},
...
function cao_cao_schemes:initialise()
	...
	effect.set_context_value("dlc07_cao_cao_schemes_list", self.schemes_data)
	self:update_pawn_ui();
end;
...
function cao_cao_schemes:setup_listeners()
	...
	core:add_listener(
		...
		function(context)			-- Function to fire			
			...
			if not context:faction():is_human() then
				...
				local war_list = cm:query_faction(self.cao_cao_faction_key):factions_we_have_specified_diplomatic_deal_with("treaty_components_war")
				if not war_list:is_empty() then
					...
					if not is_number(selected_target_index) then
						script_error("Schemes: Unable to find any valid selected_target_index" .. tostring(selected_target_index))
						return false
					end;
				...
				output(string.format("Chosen Scheme : %s, Chosen Target : %s", selected_scheme.title_key, target_faction:name()))
				...
				if target_type == "character" then
					...
					if faction_leader:is_null_interface() then
						script_error(string.format("Failed to find faction leader for faction (%s)", target_faction:name()))
						return false
					end
					
					output(string.format("Applying effect bundle %s on faction leader in faction %s", selected_scheme.effect_bundle_key, target_faction:name()))
				...
				elseif target_type == "faction" then
					output(string.format("Applying effect bundle %s on faction %s", selected_scheme.effect_bundle_key, target_faction:name()))
				...
				elseif target_type == "force" then
					...
					if not force_list:is_empty() then
						...
						if not is_number(selected_force_index) then
							script_error("Schemes: Unable to find any valid selected_force_index" .. tostring(selected_force_index))
							return false
						end
						...
						if selected_force:is_null_interface() then
							script_error(string.format("Failed to find force at index (%i) in force list for faction (%s)", selected_force_index, target_faction:name()))
							return false
						end

						--Finally apply the effect bundle on the force
						output(string.format("Applying effect bundle %s on faction force %s", selected_scheme.effect_bundle_key, force_list:item_at(selected_force_index):command_queue_index()))
				...
				elseif target_type == "province" then
					...
					if not province_list:is_empty() then
						...
						if not is_number(selected_province_index) then
							script_error("Schemes: Unable to find any valid selected_province_index" .. tostring(selected_province_index))
							return false
						end
						...
						if selected_faction_province:is_null_interface() then
							script_error(string.format("Failed to find province at index (%i) in province list for faction (%s)", selected_province_index, target_faction:name()))
							return false
						end
	...
	core:add_listener(
		...
		function(model_script_notification_event)				--Function to fire
			...
			local target_faction_key = effect.get_context_value("CcoScriptObject", "scheme_target_faction_key", "StringValue")
			...
			elseif target_type == "faction" then
				...
				if self.schemes_data[scheme_key].automatic_deal_key then
					cm:modify_faction(target_faction_key):apply_automatic_diplomatic_deal(self.schemes_data[scheme_key].automatic_deal_key, cm:query_faction("3k_main_faction_cao_cao"), "faction_key:"..target_faction_key);
				end		
			...
			elseif target_type == "province" then
				...
				if not query_faction_province or query_faction_province:is_null_interface() then
					script_error(string.format("Failed to find faction province for %s in province (%i)", target_faction_key, target_key))
				else
			...
			if string.find(scheme_key, "scheme_faction_tech") then
				local tech_cooldown = cm:query_faction(self.cao_cao_faction_key):get_tech_research_cooldown()
