---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
----- Name:			3k_campaign_cdir_events_manager.lua
----- Description: 	Three Kingdoms system to trigger events for player factions, especially when they're out of turn.
---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------

cm:add_first_tick_callback(function() cdir_events_manager:initialise() end); --Self register function
...
force_require("3k_campaign_cdir_events_data");
...
function cdir_events_manager:initialise()
	...
	-- testing. ANDREAS LOOK HERE
	-- Example: trigger_cli_debug_event dynamic_event_test()
	core:add_cli_listener("dynamic_event_test", 
		function()
			local incident = cm:modify_model():create_incident("3k_dlc07_faction_council_issue_hostile_armies_replenishing_target_incident");

			incident:add_character_target("target_character_1", cm:query_model():character_for_template("3k_main_template_historical_cao_cao_hero_earth"));
			incident:add_faction_target("target_faction_1", cm:query_faction("3k_main_faction_liu_bei"));
			incident:add_region_target("target_region_1", cm:query_region("3k_main_pingyuan_capital"));

			incident:trigger(cm:modify_faction(cm:get_local_faction(true)), true)
		end
	);
...
function cdir_events_manager:apply_events()
	...
	for i, faction_key in ipairs( human_factions ) do
		...
		-- PRIORITISED EVENTS - Unlike others we try and fire ALL of them. This diverges from the db system which only fires one.
		if self:get_num_events_for_faction_from_queue(cqi, self.queued_prioritised_incidents) > 0 or self:get_num_events_for_faction_from_queue(cqi, self.queued_prioritised_dilemmas) > 0 then
			...
			local pri_event_triggered = false;
			for i = 1, #incidents do
				pri_event_triggered = modify_faction:trigger_incident( incidents[i], true);
				if pri_event_triggered then
					event_fired = true;
				end;
			end;

			for i = 1, #dilemmas do
				pri_event_triggered = modify_faction:trigger_incident( dilemmas[i], true);
				if pri_event_triggered then
					event_fired = true;
				end;
			end;
...
function cdir_events_manager:multiplayer_event_listeners()
	...
	core:add_listener(
		...
		function(context)
			if not is_nil(self.mp_mirror_events[context:dilemma()]) then
				if not is_nil(self.mp_mirror_events[context:dilemma()][3]) then
					return self.mp_mirror_events[context:dilemma()][3] == context:choice();
				else
					return true;
				end;
			end;
