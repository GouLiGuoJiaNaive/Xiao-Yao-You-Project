---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
----- Name:			3k_campaign_experience.lua
...
cm:add_first_tick_callback(function() campaign_experience:setup_experience_triggers() end); --Self register function
...
function campaign_experience:setup_experience_triggers()
	...
	-- Example: trigger_cli_debug_event campaign_experience.add_experience(cqi,amount)
	core:add_cli_listener("campaign_experience.add_experience", 
		function(character_cqi, amount) 
			local m_char = cm:modify_character(character_cqi);

			if not m_char or m_char:is_null_interface() then
				script_error("ERROR: debug add_experience, unable to find character with this cqi.")
				return;
			end;

			self:add_experience_internal(m_char, amount, "debug");
			output(string.format("XP Added. Char: %s Amount: %s", tostring(character_cqi), tostring(amount)));
		end 
	);
