...
function cutscene:skip()
	...
	bm:callback(
		function()
			...
			if (self.stop_cindy_scene_on_finish or self.cindy_scenes_added) and self.should_stop_cindy_playback then
...
-- @r vector position offset from predicted future position of unit.
...
function get_track_commander_positions(sunit, h_camera_bearing, v_camera_bearing, camera_distance, commander_height, unit_speed, tracking_time_offset, tracking_time, change_in_terrain_height)
	...
	script_error("get_track_commander_positions ERROR: Function requires rewriting.");
	...
	local current_commander_pos = predict_commander_position(sunit.unit, unit_speed, tracking_time_offset, commander_height, 0);
	...
	local future_commander_pos = predict_commander_position(sunit.unit, unit_speed, tracking_time, commander_height, 0);
