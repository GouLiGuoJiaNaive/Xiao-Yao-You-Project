...
--- @desc Note that the standard lua version of this function supports pattern-matching. The implementation of this function in Total War does not, as it has been rewritten to be fully utf8-compliant. Use @string:match to search by pattern.
--- @p @string string, Subject string.
--- @p @string substring, String pattern to search for.
--- @p [opt=1] start index, Position of character at which to start the search. Supply a negative number to specify a character from the end of the string, counting back.
...
--- @function len
--- @desc Returns the number of characters in the supplied string.
--- @p @string input
--- @r @number length
--- @example str = "hello"
...
--- @function split
--- @desc Returns the supplied string, split into a table by a delimiter.
--- @p @string input
--- @p @string the seperator/delimiter
--- @r @table The string split by the delimiter (or the whole string in a table if not found)
--- @example string.split("thing;thing2;thing3", ";")
--- @result {"thing", "thing2", "thing3"}
string.__split_banned_delimiters = {"%", "£", "(", ")"}; -- All these throw errors in regex searches
string.__split_default_separator = ";"; -- If we have a banned separator, we'll use this as a default
function string.split(inputstr, separator)
	local t = {}; -- our returned values.

	if not separator then
		script_error("Error: string.split: No seperator passed in, returning string as table.");
		return {inputstr};
	end

	-- If delimiter is on the 'banned' list, replace the characters in both the separator and the test_string with something acceptable. 
	for i, v in ipairs(string.__split_banned_delimiters) do
		if v == separator then
			inputstr = string.gsub(inputstr, "%" .. separator, string.__split_default_separator);
			separator = string.__split_default_separator;
      
			break;
		end;
	end;

	-- Check for the string ending with our delimiter
	if string.sub(inputstr, -1) == separator then
		inputstr = string.sub(inputstr, 1, #inputstr - 1)
	end;

	-- Build a table of our values.
	for field,s in string.gmatch(inputstr, "([^"..separator.."]*)("..separator.."?)") do 

		table.insert(t,field);

		if s=="" then -- Return our table if we've not got another separator.
			return t;
		end
	end

	return {inputstr}; -- If we didn't find anything then return the input string back.
end

--- @function trim
--- @desc Returns the supplied string, with whitespace removed from the start and end.
--- @p @string input
--- @example string.trim("  string value  ")
--- @result "string value"
function string.trim(input_str)
	return string.match(input_str, "^%s*(.-)%s*$");
end;

--- @function trim_start
--- @desc Returns the supplied string, with whitespace removed from the start
--- @p @string input
--- @example string.trim("  string value  ")
--- @result "string value  "
function string.trim_start(input_str)
	return string.match(input_str, "^%s*(.+)");
end;

--- @function trim_end
--- @desc Returns the supplied string, with whitespace removed from the end
--- @p @string input
--- @example string.trim("  string value  ")
--- @result "  string value"
function string.trim_end(input_str)
	return string.match(input_str, "(.-)%s*$");
end;
...
function table.tostring(t, for_campaign_savegame, max_level, tab_level, prepend_tabs, add_line_break)
	...
	if is_function(prepend_tabs) then
		...
		if max_level ~= -1 and tab_level >= max_level then
...
--- @function contains
--- @desc Reports if the supplied table contains the supplied value. If the table does contain the supplied value, returns true, otherwise <code>false</code> is returned.
--- @p @table table, Subject table.
--- @r key key value
...
--- @function length
--- @desc Returns the length of the table. This is mainly useful for k, v tables which don't have a specified length.
--- @p @table object
--- @r @int address string
...
--- @function is_array
--- @desc Returns the true if the table is an indexed array (e.g. t = {1, 2, 3, ...}), false if it's key_value (e.g. t = {["a"]=1, ["b"]=2, ["c"]=3, ...}).
--- @p @table object
--- @r @boolean is array
...
--- @function filter
--- @desc Filters a table down by a predicate function into a new table and returns it. If the table is indexed it will return an indexed table, if it's not then it'll return a key-value table.
--- @p @table object
--- @p @function Filter function
--- @r @table A filtered table of objects.
function table.filter(t, filter_function)
	local out_tbl = {}

	for k, v in pairs(t) do
		if filter_function(v, k, t) then
			if table.is_array(t) then
				table.insert(out_tbl, v)
			else
				out_tbl[k] = v;
			end
		end;
