...
function unit_manager:unit_carrying_siege_equipment() 
    ...
    for i = 1, player_units:count() do
        ...
        local carrying_equipment = effect.get_context_value("CcoBattleUnit", unit_id, "VehicleContext");
...
function unit_manager:get_armour_key(unit) 
    ...
    local armour_key = effect.get_context_value("CcoBattleUnit", object_id_no, "UnitRecordContext.ArmourRecordContext.Key");
    return armour_key;
...
function unit_manager:get_minimum_distance_to_capture_point() 
    ...
    local x,y,z,w = effect.get_context_value("CcoBattleRoot", "", "VictoryCapturePointContext.Position");
...
function unit_manager:getUnitWithCEORarity(rarity_value)
    ...
    for i = 1, player_units:count() do
        ...
        if (effect.get_context_value("CcoBattleUnit", unit_id, "SetupUnitContext.CeoList.MaxValue(RarityValue)") == rarity_value) then 
