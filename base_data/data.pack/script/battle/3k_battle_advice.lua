...
force_require("3k_battle_advice_phases");
force_require("3k_battle_advice_chains");
force_require("3k_battle_advice_timing_manager");
force_require("3k_battle_advice_logger");
force_require("3k_battle_advice_unit_manager");
...
function battle_advice_system:add_victory_point_context_object(advice_monitor, wait_time) 
	...
	local object_id = effect.get_context_value("CcoBattleRoot", "", "VictoryCapturePointContext");
	...
	if object_id then
		...
		battleAdviceLogger:log("add_victory_point_context_object() Not adding as there is no VictoryCapturePointContext. This likely means it's an unwalled siege, but if it's not, that's a problem. Advice Monitor [" .. tostring(advice_monitor.name) .. "]");
	end;
