...
function intervention_manager:start_context_change_monitor()
	...
	local function context_changed(reason)
		...
		if self.intervention_currently_active then
			...
			self:out("intervention_manager:start_context_change_monitor() is responding to a change of context but no intervention is currently active to complete - is this valid?");
		end;
...
--- @example 	"ScriptEventHumanFactionTurnStart",
...
function intervention:complete()
	...
	-- Only attempt to stop it if it's active.
	if self.is_started then
		self:stop();
	end;
...
function intervention:start_restart_listeners()
	...
	for i = 1, #restart_checks do
		local restart_entry = restart_checks[i];
		
		core:add_listener(
			self.restart_listener_name,
			restart_entry.event_name,
...
function intervention:trigger_ui_context_for_duration(context_key, enable_value, disable_value, duration)
			...
			cm:callback(
				function() 
					if self.intervention_system_debug then
						self:out("");
						self:out(separator_str);
						self:out("trigger_ui_context_for_duration() :: Setting ui context " .. context_key .. " to " .. disable_value);
						self:out(separator_str);
					end;
	...
	if self.intervention_system_debug then
		self:out("");
		self:out(separator_str);
		self:out("trigger_ui_context_for_duration() :: Setting ui context " .. context_key .. " to " .. enable_value .. "for " .. tostring(duration) .. "seconds.");
		self:out(separator_str);
	end;
