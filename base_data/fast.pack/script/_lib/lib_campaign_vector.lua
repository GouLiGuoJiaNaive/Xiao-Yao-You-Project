...
function campaign_vector:distance(target)
	if not is_vector(target) then
		script_error("ERROR: campaign_vector:distance() called but supplied target [" .. tostring(target) .. "] is not a vector");
		return false;
	end;
...
function campaign_vector:distance_xz(target)
	if not is_vector(target) then
		script_error("ERROR: campaign_vector:distance_xz() called but supplied target [" .. tostring(target) .. "] is not a vector");
		return false;
	end;
