...
core_object = {
	...
	-- True while callbacks are processing, false otherwise. Guard against removing callbacks while they're iterating. 
...
function core_object:event_callback(eventname, context)
	...
	for i = 1, #listeners do
		...
		-- Wrapping the condition call in a pcall wrapper so it doesn't break execution. 
		local ret_val = nil;
		if is_function(current_listener.condition) then
			ret_val = self:pcall_with_errors(current_listener.condition, context);
		else
			ret_val = current_listener.condition;
		end
		
		if not current_listener.to_remove and ret_val then
	...
	self:clean_listeners(eventname);
	
	-- Call the callbacks within pcall so they are protected.
	for i = 1, #callbacks_to_call do
		self:pcall_with_errors(callbacks_to_call[i], context);
	end;
	...
	for event_name, listeners in pairs(self.event_listeners) do
		for i = #listeners, 1, -1 do
			if listeners[i].name == name_to_remove then
				if self.is_processing_callbacks then
					listeners[i].to_remove = true;
				else
...
--- @desc By convention, the names of events triggered from script are prepended with "ScriptEvent" e.g. "ScriptEventHumanFactionTurnStart".
--- @p @string event name
		...
		if not force_wait_for_loading_screen_dismissed_in_campaign and uic_loading_screen:CurrentAnimationId() ~= "" then
			...
			core:add_listener(
				...
				function()
					...
					if uic_loading_screen == nil or suppress_wait_for_animation then
					...
					out("=== progress_on_loading_screen_dismissed() - loading screen with name [" .. loading_screen_name .. "] has been dismissed, waiting for it to finish animating [" .. uic_loading_screen:CurrentAnimationId() .. "]");
					-- loading screen is animating to hide	
					self:progress_on_uicomponent_animation_finished(
						uic_loading_screen,
						function()
...
function core_object:strip_tags_from_localised_text(localised_text)
	...
	for i = 1, #localised_text_tags do
		start_tag = localised_text_tags[i].start_tag;
...
--- @page core
--- @desc A custom context is created when the core object is compelled to trigger an event with <code>trigger_event</code>. Data items supplied to @core_object:trigger_event or @core_object:trigger_custom_event are added to the custom context, which is then sent to any script listening for the event being triggered.
...
--- @desc No script outside of the @core_object:trigger_event function should need to create a custom context, but it's documented here in order to list the data types it supports.
--- @desc The @core_object:trigger_custom_event function allows triggering script to customise the function names on the custom context by which the receiving script may query the context data.
----------------------------------------------------------------------------
----------------------------------------------------------------------------
...
--- @desc &emsp;boolean: will be accessible to the receiving script as <code>context.bool</code>
...
--- @desc &emsp;diplomacy_component: will be accessible to the receiving script using <code>context:diplomacy_component()</code>
...
function custom_context:add_data(obj)
	...
	elseif is_query_diplomacy_proposed_component(obj) or is_query_diplomacy_deal_component(obj) then
		self.diplomacy_component_data = obj;
...
--- @function diplomacy_component
--- @desc Called by the receiving script to retrieve the diplomacy_component object placed on the custom context, were one specified by the script that created it.
--- @r diplomacy_component diplomacy_component object
function custom_context:diplomacy_component()
	return self.diplomacy_component_data;
end;
...
function core_object:initialise_cli_listeners()
        ...
        function(event)
			...
			if string.match(event_parameter, "%(") then -- Check if there is an opening parenthesis in the string.
				...
				for value in string.gmatch(remaining_params, "[^,%s]+") do -- A Comma
					...
					-- Convert the value to different types.
					if value == "true" then
						value = true;
			...
			output("cli_listener: Calling " .. event_key .. " with " .. #args_list .. " argument(s).");
	...
	-- Example: trigger_cli_debug_event trigger_event(key)
	self:add_cli_listener("trigger_event", 
		function(event_key) 
			self:trigger_event(event_key);
			output("Event Triggered" .. tostring(event_key));
		end 
	);

	-- Example: trigger_cli_debug_event help()
	self:add_cli_listener("help", 
		function()  
			...
			for i, v in ipairs(self.cli_listeners) do
				output( "trigger_cli_debug_event " .. tostring( v[1] ) .. "(params)" ); -- Output the name of the event.
			end;
			dec_tab();
			output("*********");
		end
	...
	-- Example: trigger_cli_debug_event debug_event_listeners()
	self:add_cli_listener("debug_event_listeners",
		function(is_enabled)
			if is_enabled == nil then
				self.debug_events = not self.debug_events;
			else
				self.debug_events = is_enabled;	
			end
			
			output("^_^ Debug Events: " .. tostring(self.debug_events));
		end
	)
end;
...
--- @function pcall_with_errors
--- @desc Calls the supplied function in such a way as it won't break execution of subsequent scripts. Takes a variable number of args and also fires script errors automagically.
function core_object:pcall_with_errors(func, ...)
	local ok, result = pcall(func, ...);

	if not ok then
		script_error("[pcall]" .. tostring(result));
		return false;
	end;
