...
TYPE_FE_HB_SEQUENCE = "frontend sequence";
...
elseif __game_mode == __lib_type_campaign then
	...
	force_require("lib_campaign_pending_battle_cache"); -- Added in 3K DLC05.
	force_require("lib_objectives");
	force_require("lib_infotext");
	force_require("lib_help_pages");
	force_require("lib_convex_area");
	force_require("lib_campaign_string_mission") -- Handles creation and triggering for string missions - mission built within the script.
...
set_all_scripted_core(core);
set_all_scripted_campaign_ui(CampaignUI);
