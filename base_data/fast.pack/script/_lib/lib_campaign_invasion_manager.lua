...
--
---	@loaded_in_campaign
---	@c invasion_manager invasion_manager Invasion Manager
...
function invasion_manager:new_invasion(key, faction_key, force_list, spawn_location)
	self:print("Invasion Manager: New Invasion '" .. tostring(key) .. "'");
	self:print("\tFaction: "..tostring(faction_key));
	self:print("\tForce: "..tostring(force_list));
	self:print("\tSpawn: "..tostring(spawn_location));
...
function invasion_manager:new_invasion_from_existing_force(key, force)
	self:print("Invasion Manager: New Invasion from existing force'"..tostring(key).."'...");
	self:print("\tForce: "..tostring(force));
...
function invasion_manager:parse_spawn_location(spawn_location)
	self:print("Invasion Manager: Parsing a Spawn Location of '"..tostring(spawn_location).."'");
...
function invasion_manager:remove_invasion(key)
	if key ~= nil then
		self:print("Invasion Manager: Removing Invasion '"..key.."'");
...
function invasion:force_created(general_cqi, declare_war, invite_attacker_allies, invite_defender_allies, was_respawn)
	...
	if self.target_faction ~= nil then
		...
		if this_faction and enemy_faction and this_faction ~= enemy_faction then
			if declare_war and not diplomacy_manager:is_at_war_with(this_faction:name(), enemy_faction:name()) then
				diplomacy_manager:force_declare_war(self.faction, self.target_faction, self.event_feed_suppressed);
	...
	if force:is_null_interface() == false then
		...
		if general:is_null_interface() == false then
			...
			if self.target_type ~= "NONE" then
				...
				if self.target_faction ~= nil then
					if self.faction ~= self.target_faction then
						invasion_manager:print("\tOn advance, "..self.faction.." declares war on "..self.target_faction);
						diplomacy_manager:force_declare_war(self.faction, self.target_faction, true);
					elseif self.aggro_target_cqi ~= nil then
						self.aggro_target_cqi = nil
					end
				end
				...
				if self.aggro_enabled == true and self.aggro_target_cqi ~= nil then
					...
					if char_obj:is_null_interface() == false then
						...
						diplomacy_manager:force_declare_war(char_obj:faction():name(), general:faction():name(), false, false);
				...
				elseif self.target_type == "REGION" then
					...
					if target_region_obj:is_null_interface() == false and target_region_obj:is_abandoned() == false then
						...
						if region_owner == self.target_faction then
							invasion_manager:print("\tAttacking Region  (" .. target_region_obj:name() .. ")");
							cm:modify_campaign_ai():cai_disable_movement_for_character(general_lookup);
							modify_character:replenish_action_points();
							
							if modify_character:attack_settlement(target_region_obj:settlement()) == false then
								invasion_manager:print("\tCouldn't attack region, releasing force!");
								self.target_type = "NONE";
							end
						else
							...
							if self.target_owner_abort == false then
								...
								if self.faction == self.target_faction then
									invasion_manager:print("\ttarget is the same faction as the invading force's faction - Aborting Invasion!");
									self.target_type = "NONE";
								else
									invasion_manager:print("\ttarget_owner_abort is "..tostring(self.target_owner_abort).." - Switching Target Faction!");
									diplomacy_manager:force_declare_war(self.faction, self.target_faction, true, true);
								end
							else
				...
				elseif self.target_type == "FORCE" then
					...
					if target_force_obj:is_null_interface() == false then
						if target_force_obj:has_general() == true then
							...
							invasion_manager:print("\tAttacking Force (" .. enemy_general_cqi ..")");
							cm:modify_campaign_ai():cai_disable_movement_for_character(general_lookup);
							modify_character:replenish_action_points();

							if modify_character:attack(target_force_obj:general_character()) == false then
								invasion_manager:print("\tCouldn't attack force, releasing force!");
								self.target_type = "NONE";
							end
						end
					else
...
function invasion:find_aggro_target() -- TODO: might want to return the weakest force here, as currently will return the first one it finds
	...
	if general:is_null_interface() == false then
		...
		if self.aggro_targets[1] == "war" then
			local faction_list = diplomacy_manager:factions_at_war_with(general:faction():name());
			self.aggro_targets = {};
