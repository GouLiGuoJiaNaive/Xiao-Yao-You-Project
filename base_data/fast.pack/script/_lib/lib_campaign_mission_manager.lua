...
--	Definition
----------------------------------------------------------------------------
mission_manager_save_version_official = 2;
mission_manager_save_version_current = 1;
...
mission_manager = {
	...
	-- for dilemmas
	dilemma_choice_callback_first = nil,
	dilemma_choice_callback_second = nil,
	dilemma_choice_callback_third = nil,
	dilemma_choice_callback_fourth = nil,

	event_trigger_callback = nil,
...
function mission_manager:new(faction_name, mission_key, success_callback, failure_callback,cancellation_callback, nearing_expiry_callback, optional_turn_limit)
	...
	if optional_turn_limit and not is_number(optional_turn_limit) then
		script_error("ERROR: mission_manager:new() called but supplied turn limit [" .. tostring(optional_turn_limit) .. "] is not a number or nil");
		return false
	end
	...
	mm.turn_limit = optional_turn_limit;
	mm.objectives = {};
	mm.scripted_objectives = {};
	
	mm.dilemma_choice_callback_first = nil;
	mm.dilemma_choice_callback_second = nil;
	mm.dilemma_choice_callback_third = nil;
	mm.dilemma_choice_callback_fourth = nil;
	mm.fire_immediately = true;
...
function mission_manager:register()
	if not self.is_registered then
		...
		if cm:get_mission_manager(self.mission_key, self.faction_name) then
...
function mission_manager:add_dilemma_choice_callback(choice_id, callback)
	
	if not is_number(choice_id) then
		script_error("ERROR: add_dilemma_choice_callback() called on mission manager for mission key [" .. self.mission_key .. "] but supplied choice_id [" .. tostring(choice_id) .. "] is not a number");
		return false;
	end;

	if not is_function(callback) and not is_string(callback) then
		script_error("ERROR: ERROR: add_dilemma_choice_callback() called on mission manager for mission key [" .. self.mission_key .. "] but supplied callback [" .. tostring(callback) .. "] is not a function or a string");
		return false;
	end;

	if choice_id == 0 then
		self.dilemma_choice_callback_first = callback;
	elseif choice_id == 1 then
		self.dilemma_choice_callback_second = callback;
	elseif choice_id == 2 then
		self.dilemma_choice_callback_third = callback;
	elseif choice_id == 3 then
		self.dilemma_choice_callback_fourth = callback;
	else
		script_error("ERROR: add_dilemma_choice_callback() called on mission manager for mission key [" .. self.mission_key .. "] but supplied choice_id [" .. tostring(choice_id) .. "] is greter than 3");
		return false;
	end;
end;

function mission_manager:add_on_trigger_callback(callback)
	if not is_function(callback) then
		script_error("ERROR: ERROR: add_dilemma_choice_callback() called on mission manager for mission key [" .. self.mission_key .. "] but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;

	self.event_trigger_callback = callback;
end;
...
function mission_manager:set_should_fire_immediately(value)
	if not is_boolean(value) then
		script_error("ERROR: set_should_fire_immediately() called but supplied condition (" .. tostring(value) .. ") is not a boolean");
		return false;
	end;
...
function mission_manager:trigger(dismiss_callback, delay)
	...
	if self.should_trigger_from_string then
		-- build a mission string object.
		local new_string_mission = string_mission:new(self.mission_key);

		if self.mission_issuer then
			new_string_mission:set_issuer(self.mission_issuer);
		end;

		if self.chapter_mission then
			new_string_mission:set_chapter(self.chapter_mission);
		end;

		if self.turn_limit then
			new_string_mission:set_turn_limit(self.turn_limit);
		end;
		...
		for i = 1, #self.objectives do
			...
			if i == 1 then
				new_string_mission:add_primary_objective(current_objective.objective_type, current_objective.conditions);
				for i, v in ipairs(current_objective.payloads) do
					new_string_mission:add_primary_payload(v);
				end;
			else
				new_string_mission:add_secondary_objective(current_objective.objective_type, current_objective.conditions, current_objective.payloads)
			end;
		end;
		
		if not new_string_mission then
			script_error("ERROR: trigger() called on mission manager with key [" .. self.mission_key .. "] but failed to construct a mission string");
			return false;
		else
			self.string_mission_obj = new_string_mission;
		end;
	end;
	...
	elseif self.should_trigger_from_db then
		...
		if self.is_mission then
			local success = self.cm:trigger_mission(self.faction_name, self.mission_key, self.fire_immediately, self.should_whitelist);

			-- if the mission failed to trigger, then we assume failure.
			if not success and is_function(self.failure_callback) then
				if self.force_immediate_callback then
					self.failure_callback();
				else
					cm:progress_on_battle_completed(
						"mm_" .. self.mission_key,
						function()
							self.failure_callback();
						end
					);
		...
		elseif self.is_incident then
			local success = self.cm:trigger_incident(self.faction_name, self.mission_key, self.fire_immediately, self.should_whitelist);

			-- Trigger failure events for incidents. success will return false if the incident was unable to fire.
			if not success then
				self:complete();
		...
		elseif self.is_dilemma then
			local success = self.cm:trigger_dilemma(self.faction_name, self.mission_key, self.fire_immediately, self.should_whitelist);

			--  Trigger failure events for dilemmas. success will return false if the dilemma was unable to fire.
			if not success then
				self:complete();
...
function mission_manager:cancel_mission()

	if not self.started then
		script_error("ERROR: cancel_mission() called on mission manager with key [" .. self.mission_key .. "] but the mission hasn't been triggered.");
		return false;
	end
	
	if self.completed then
		script_error("ERROR: cancel_mission() called on mission manager with key [" .. self.mission_key .. "] but the mission has finished.");
		return false;
	end;

	if not self.is_dilemma or not self.is_incident then
		script_error("ERROR: cancel_mission() called on mission manager with key [" .. self.mission_key .. "] but this is not a mission.");
		return false;
	end;

	-- Cancel custom missions using the existing system.
	if not self.should_trigger_from_db then
		cm:cancel_custom_mission(self.faction_name, self.mission_key);

	-- Cancel db missions using their cqi.
	elseif self.mission_cqi then
		local modify_mission = cm:modify_model():get_modify_mission_by_cqi(self.mission_cqi);

		if not modify_mission or modify_mission:is_null_interface() then
			script_error("ERROR: cancel_mission() Unable to get mission with cqi " .. tostring(self.mission_cqi));
			return;
		end;
		...
		script_error("ERROR: cancel_mission() called on mission manager with key [" .. self.mission_key .. "] but the mission fired from db but has no cqi stored. This shouldn't happen.");
	end;
end;
...
function mission_manager:trigger_from_string()
	...
	self.string_mission_obj:trigger_mission_for_faction(self.faction_name);
end;
...
function mission_manager:start_listeners()
	...
	if self.is_mission then
		...
		-- issued listener
		core:add_listener(
			mission_key .. "_issued_listener",
			"MissionIssued",
			function(context) return context:mission():mission_record_key() == mission_key end,
			function(context)
				output("~~~ MissionManager :: " .. mission_key .. " issued event received");
				
				if is_function(self.event_trigger_callback) then
					self.event_trigger_callback();
				end;

				self.mission_cqi = context:mission():cqi();
			end,
			false
		);
	...
	elseif self.is_incident then
		...
		core:add_listener(
			...
			function(context) return context:incident() == mission_key end,
			function(context)
				output("~~~ MissionManager :: " .. mission_key .. " issued event received");
	...
	elseif self.is_dilemma then
		...
		core:add_listener(
			mission_key .. "_success_listener",
			"DilemmaIssuedEvent",
			function(context) return context:dilemma() == mission_key end,
			function()
				output("~~~ MissionManager :: " .. mission_key .. " issued event received");
		...
		-- Add listeners for player choice (SM)
		core:add_listener(
			mission_key .. "_choice_listener",
			"DilemmaChoiceMadeEvent",
			function(context) return context:dilemma() == mission_key end,
			function(context)
				local choice = context:choice();

				output("~~~ MissionManager :: " .. mission_key .. " choice event received choice [" .. tostring(choice) .. "]");

				if choice == 0 then
					if is_function(self.dilemma_choice_callback_first) then
						self.dilemma_choice_callback_first();
					elseif is_string(self.dilemma_choice_callback_first) then
						core:trigger_event(self.dilemma_choice_callback_first);
					end;
				elseif choice == 1 then
					if is_function(self.dilemma_choice_callback_second) then
						self.dilemma_choice_callback_second();
					elseif is_string(self.dilemma_choice_callback_second) then
						core:trigger_event(self.dilemma_choice_callback_second);
					end;
				elseif choice == 2 then
					if is_function(self.dilemma_choice_callback_third) then
						self.dilemma_choice_callback_third();
					elseif is_string(self.dilemma_choice_callback_third) then
						core:trigger_event(self.dilemma_choice_callback_third);
					end;
				elseif choice == 3 then
					if is_function(self.dilemma_choice_callback_fourth) then
						self.dilemma_choice_callback_fourth();
					elseif is_string(self.dilemma_choice_callback_fourth) then
						core:trigger_event(self.dilemma_choice_callback_fourth);
					end;
				end;
			end,
			false
...
function mission_manager:state_to_string()
	return self.mission_key .. "%" .. tostring(self.started) .. "%" .. tostring(self.completed) .. "%" .. self.faction_name .. "%";
end;
...
function mission_manager:complete()
	...
	core:remove_listener(mission_key .. "_issued_listener");
