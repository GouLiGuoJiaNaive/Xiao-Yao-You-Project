...
function help_page_manager:new()
	...
	table.insert(hide_on_event, "ScriptEventLocalPlayerFactionTurnEnd");
	
	local attempt_show_on_event = {};
	table.insert(attempt_show_on_event, "ScriptEventPlayerBattleCompletedSP");
	table.insert(attempt_show_on_event, "ScriptEventLocalPlayerFactionTurnStart");
...
function help_page_manager:help_page_index_generated()
	...
	for i = 0, uic_list:ChildCount() - 1 do
		...
		local unparsed_text, source_key = uic_entry:GetStateText();
		local parsed_text = parser:parse_for_links(unparsed_text);
		
		uic_entry:SetStateText(parsed_text, source_key);
	end;
...
function help_page:append_help_page_record(record)
	...
	if not record.game_guide or (record.game_guide and hpm:display_game_guide_links()) then
			...
			if record.show_modifier then
				...
				if uic_modifier then
					if record.show_drag_modifier then
						uic_modifier:SetStateText(effect.get_localised_string("ui_text_replacements_localised_text_hp_battle_cheat_sheet_drag_modifier"), "ui_text_replacements_localised_text_hp_battle_cheat_sheet_drag_modifier");
					
					elseif record.show_roll_modifier then
						uic_modifier:SetStateText(effect.get_localised_string("ui_text_replacements_localised_text_hp_battle_cheat_sheet_roll_modifier"), "ui_text_replacements_localised_text_hp_battle_cheat_sheet_roll_modifier");
					
					elseif record.show_hold_modifier then
						uic_modifier:SetStateText(effect.get_localised_string("ui_text_replacements_localised_text_hp_battle_cheat_sheet_hold_modifier"), "ui_text_replacements_localised_text_hp_battle_cheat_sheet_hold_modifier");
					
					elseif record.show_on_enemy_modifier then
						uic_modifier:SetStateText(effect.get_localised_string("ui_text_replacements_localised_text_hp_battle_cheat_sheet_on_enemy_modifier"), "ui_text_replacements_localised_text_hp_battle_cheat_sheet_on_enemy_modifier");
					end;
		...
		local localised_text, text_source_key = uic_entry:GetStateText();
		...
		uic_entry:SetStateText(localised_text, text_source_key);
	...
	if not record.is_howto or record.current_state ~= state then
		...
		if make_active then
			...
			if record.is_active then	
				local localised_text, text_source_key = uic:GetStateText()
				...
				uic:SetStateText(core:strip_tags_from_localised_text(localised_text), text_source_key);
			end;
...
function link_parser:new()
	...
	lp.parse_for_links_func = function(key, text)
		...
		if effect.get_advice_history_string_seen(script_link) then
			...
			return "[[url_visited:" .. link_record.script_link .. "]][[tooltip:{{tt:" .. link_record.tooltip_key .. "}}]]" .. text .. "[[/tooltip]][[/url]]";
		end;
	end;
...
function link_parser:parse_component_for_links(uic)
	...
	local component_text, text_source_key = uic:GetStateText();
	local parsed_text = self:parse(component_text, self.parse_for_links_func);
	
	if parsed_text then
		uic:SetStateText(parsed_text, text_source_key);
	end;
end;
...
function link_parser:parse_component_for_tooltips(uic)
	...
	local component_text, text_source_key = uic:GetStateText();
	local parsed_text = self:parse(component_text, self.parse_for_tooltips_func);
...
function tooltip_patcher:new(layout_key)
	...
	if not is_string(layout_key) then
		script_error("ERROR: add_tooltip_patcher() called but supplied layout key [" .. tostring(layout_key) .. "] is not a string");
		return false;
	end;
