...
function campaign_manager:set_saved_value(key, value, ...)
	...
	if not (is_string(value) or is_number(value) or is_boolean(value) or is_table(value)) then
...
function campaign_manager:wait_for_model_with_id(id, callback, persistent)
	...
	core:add_listener(
		"campaign_manager_wait_for_model_sp_" .. id,
		"CampaignModelScriptCallback",
		function(context)
		...
		function(context)
			if not persistent then
				core:remove_listener("campaign_manager_wait_for_model_sp_" .. id);
			end;
