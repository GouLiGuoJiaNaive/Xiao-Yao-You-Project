...
function char_is_army_commander(char)
	if not is_query_character(char) then
		script_error("ERROR: char_is_army_commander() called but supplied object [" .. tostring(char) .. "] is not a character");
		return false;
	end;
...
function get_trespasser_list_for_faction(faction)
	...
	for i = 0, faction_list:num_items() - 1 do
		...
		if diplomacy_manager:is_at_war_with(faction, current_faction) then
		...
		if current_mf:has_general() and not current_mf:is_armed_citizenry() then
			...
			if char:has_region() then			
				...
				if not region:is_abandoned() then
					...
					if not owning_faction:is_null_interface() and diplomacy_manager:is_at_war_with(owning_faction, faction) then
						return char;
					end;
				end;
			end;
