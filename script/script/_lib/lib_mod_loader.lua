...
local function get_current_time()
    return os.date("%H_%M_%d_%m_%Y");
end

if __game_mode < 0 then
	ModLog = function(...) 
        mod_loader_logger():out(...) 
	end
else
	--- @function ModLog
	...
	function ModLog(text)
		...
		if logMade == false then
            ...
            local _, err_str = io.open( "logs", 'r' )
            if err_str:match( 'No such file or directory' ) then
                os.execute( "mkdir logs" )
            end
            
			if file_exists("lua_mod_log.txt") then
                local new_file_name = "logs/lua_mod_log_" .. get_current_time() .. ".txt";
                os.rename("lua_mod_log.txt", new_file_name);
			end
			
            logInterface = io.open("lua_mod_log.txt", "w");
