...
function script_unit:new(new_army, new_ref)
	...
	if pcall(function() new_unit = new_army:units():item(new_ref) end) then
		...
		for i = 1, new_army:num_reinforcement_units() do
			...
			if pcall(function() new_unit = current_r_units:item(reinforcing_ref) end) then
				if is_number(reinforcing_ref) then
					new_ref = "R_" .. tostring(i) .. "_" .. reinforcing_ref; reinf = true;
				end;
				-- call succeeded
				unit_found = true;
				break;
	...
	if self.alliance_num == 2 then
		self.enemy_alliance_num = 1;
	else
		self.enemy_alliance_num = 2;
	end;
    ...
    -- for ai general
	su.unitclass = new_unit:unit_class();
	
	if new_unit:unit_class() == "art_siege" then
        su.is_artillery = true;
	else
        su.is_artillery = false;
	end
	if new_unit:unit_class() == "com" then
        su.is_general = true;
	else
        su.is_general = false;
	end
...
--- @p boolean activate
function script_unit:change_behaviour_active(behaviour, value)
	...
	if uic_parent and uic_parent:Visible() then	
		for i = 0, uic_parent:ChildCount() - 1 do
			...
			if uic_card:Id() == unique_ui_id then				
				local pulse_strength_to_use = pulse_strength or uim:get_panel_pulse_strength();
	...
	if unit_id_parent then 
		...
		if unit_id then
			...
			if script_ping_parent then
				local uic_ping_marker = UIComponent(script_ping_parent:CreateComponent(self.name .. "_ping_icon", "ui/battle ui/adc_ping.twui.xml"));
