...
-- 抽卡池的up角色  卡夫卡：hlyjcj  芙宁娜：hlyjco  符玄：hlyjcp
local xyy_character_up_pool = {'hlyjcj', 'hlyjco', 'hlyjcp'};

local all_character_detils = { 
    ['hlyjch'] = {['name']="钟离", ['subtype']='3k_general_earth'},
    ['hlyjci'] = {['name']="甘雨", ['subtype']='3k_general_water'},
    ['hlyjcj'] = {['name']="卡芙卡", ['subtype']='3k_general_fire', ['movie']='ep_faction_game_lost_sima_ai'},
    ...
    ['hlyjco'] = {['name']="芙宁娜", ['subtype']='3k_general_water', ['movie']='3k_main_game_lost_yellow_turbans'},
    ['hlyjcp'] = {['name']="符玄", ['subtype']='3k_general_earth', ['movie']='3k_main_game_lost_earth_yuan_shu'},
...
local function getRandomValue(min,max)
    math.randomseed(os.time()*tostring(os.clock()):reverse():sub(1, 7), tostring(os.time()):reverse():sub(1, 7)*os.clock()*1000);
...
local function remove_character_from_pool(list, key)
...
local function in_xyy_character_pool(list, value)
                ...
                if player_faction:treasury() >= cost_money or ticket_points > 0 then
                    if btn_name == UI_MOD_NAME .. "_store_buy_btn16" then
                        ...
                        if guaranteed <= 49 then
                            min = math.floor(1000 * (48 - guaranteed) /48)
                        end
                                     
                        local random = math.floor(getRandomValue(min,1000))
                        ...
                        then
                            local random = math.floor(getRandomValue(min,1050))
                        end
                        ...
                        elseif random > 940 and random < 979 then
                            items = random_item3_accessory;
                        elseif random >= 979 then
                            -- ModLog("中了")
                            ...
                            cm:wait_for_model_sp(function()
                                local function add_character_to_player(char)
                                    if all_character_detils[char]['movie'] then
                                        cm:modify_model():get_modify_episodic_scripting():register_instant_movie_by_record(all_character_detils[char]['movie']);
                                    end
                                    local character = xyy_character_add(char, player_own_modify_faction:query_faction():name(), all_character_detils[char]['subtype']);
                                    xyy_character_close_agent(char);
                                    cm:modify_character(character):reset_skills();
                                    incident = cm:modify_model():create_incident("summon_" .. char);
...
local function togglePanelVisible()
    if creaded_pannel == -1 then
        ModLog( "创建面板 " .. creaded_pannel )
        return;
    end
    if not get_xyy_character_lottery_pool() then 
    ...
    function(context)
        ...
        if player_query_faction then
            ...
            for char_id, char_info in pairs(all_character_detils) do
                ...
                ModLog('开始检测角色 '..char_id)
                if not in_xyy_character_pool(xyy_character_lottery_pool, char_id) then
                    ModLog('角色'..char_info['name']..'不在奖池')
                    if not query_character
                    or query_character:is_null_interface()
                    and not in_xyy_character_browser_list(char_id)
                    and not in_xyy_character_pool(xyy_character_up_pool, char_id) then
                        ModLog( char_info['name'] .. "不存在世界上，将添加进备选池");
                ...
                then
                    ...
                    remove_character_from_pool(xyy_character_lottery_pool, char_id);
...
core:add_listener(
    ...
    function(context)
        local faction = context:faction();
        local random = cm:random_int(0, 1000)
        ...
        local function xyy_character_official(char_id, char_detil)
            ...
            if not character or character:is_null_interface() then
                character = xyy_character_add(char_id, context:faction():name(), char_detil['subtype']);
                ModLog('刚刚' .. all_character_detils[char_id]['name'] .. "加入" .. context:faction():name())
                -- 角色特殊处理阶段
                -- 如果角色是那维莱特，设置和芙宁娜的关系
                if char_id == "hlyjcl" then
                    local furina = cm:query_model():character_for_template("hlyjco");
                    if furina and not furina:is_null_interface() then
                        cm:modify_character(furina):apply_relationship_trigger_set(character, "3k_dlc05_relationship_trigger_set_startpos_romance");
                    end
                -- 如果角色是芙宁娜，设置和那维莱特的关系
                elseif char_id == "hlyjco" then
                    local neuvillette = cm:query_model():character_for_template("hlyjcl");
                    if neuvillette and not neuvillette:is_null_interface() then
        ...
        -- 从待入仕角色中随机选一个
        local character_id = character_browser_list[getRandomValue(1, #character_browser_list)]
...
function character_browser()
    if not cm:get_saved_value("selected_up_character") then
        ModLog('没有选过角色，备选池长度 ' .. #character_browser_list)
        for i,v in ipairs(xyy_character_up_pool) do
    ...
    -- -- 设置角色抽取池的永久up角色
    -- local function set_slot_permanent(index)
    --     ModLog('初始化第'..index..'个永久up槽位 角色id ' .. xyy_character_up_pool[index] )
    --     local slot = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/".. xyy_character_up_pool[index] ..".png", effect.get_localised_string("mod_xyy_character_browser_".. xyy_character_up_pool[index]), xyy_character_up_pool[index], true, false)
    --     return slot
    -- end

    -- -- 设置角色抽取池的本期up角色
    -- local function set_slot_changeable(index)
        
    --     if xyy_character_lottery_pool[index] then
    --         ModLog('初始化第'..index..'个本期up槽位 角色id' .. xyy_character_lottery_pool[index])
    ...
    --     else
    --         ModLog('初始化第'..index..'个本期up槽位 不存在角色')
    ...
    --     if i < 4 then
    --         ModLog("创建第 "..i.." 个永久槽位")
    --         slot_list[i] = set_slot_permanent(i)
    --         UIComponent_move_relative(slot_list[i], xyy_select_character_panel, xoffset+180*i, yoffset, false)
    --     else
    --         ModLog("创建第 "..i.." 个可选槽位")
    --         slot_list[i] = set_slot_changeable(i-3)
    --         local x_p = xoffset+180*i
    --         ModLog(x_p)
    --         UIComponent_move_relative(slot_list[i], xyy_select_character_panel, x_p, yoffset, false)
    --     end
    -- end
    ModLog('up角色池长度'..#xyy_character_up_pool)
    local slot1 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/".. xyy_character_up_pool[1] ..".png", effect.get_localised_string("mod_xyy_character_browser_".. xyy_character_up_pool[1]), xyy_character_up_pool[1], true, false)
    UIComponent_move_relative(slot1, xyy_select_character_panel, xoffset+180*1, yoffset, false)
    
    local slot2 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/".. xyy_character_up_pool[2] ..".png", effect.get_localised_string("mod_xyy_character_browser_".. xyy_character_up_pool[2]), xyy_character_up_pool[2], true, false)
    UIComponent_move_relative(slot2, xyy_select_character_panel, xoffset+180*2, yoffset, false)
    
    local slot3 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/".. xyy_character_up_pool[3] ..".png", effect.get_localised_string("mod_xyy_character_browser_".. xyy_character_up_pool[3]), xyy_character_up_pool[3], true, false)
    UIComponent_move_relative(slot3, xyy_select_character_panel, xoffset+180*3, yoffset, false)
    
    if xyy_character_lottery_pool[1] then
        local slot4 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/".. xyy_character_lottery_pool[1] ..".png", effect.get_localised_string("mod_xyy_character_browser_".. xyy_character_lottery_pool[1]), xyy_character_lottery_pool[1], false, true)
        UIComponent_move_relative(slot4, xyy_select_character_panel, xoffset+180*4, yoffset, false)
    else
        local slot4 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/placeholder.png", effect.get_localised_string("mod_xyy_character_browser_unknown"), "unknown", false, false)
        UIComponent_move_relative(slot4, xyy_select_character_panel, xoffset+180*4, yoffset, false)
    end
    
    if xyy_character_lottery_pool[2] then
        local slot5 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/".. xyy_character_lottery_pool[2] ..".png", effect.get_localised_string("mod_xyy_character_browser_".. xyy_character_lottery_pool[2]), xyy_character_lottery_pool[2], false, true)
        UIComponent_move_relative(slot5, xyy_select_character_panel, xoffset+180*5, yoffset, false)
    else
        local slot5 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/placeholder.png", effect.get_localised_string("mod_xyy_character_browser_unknown"), "unknown", false, false)
        UIComponent_move_relative(slot5, xyy_select_character_panel, xoffset+180*5, yoffset, false)
    end
    
    if xyy_character_lottery_pool[3] then
        local slot6 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/".. xyy_character_lottery_pool[3] ..".png", effect.get_localised_string("mod_xyy_character_browser_".. xyy_character_lottery_pool[3]), xyy_character_lottery_pool[3], false, true)
        UIComponent_move_relative(slot6, xyy_select_character_panel, xoffset+180*6, yoffset, false)
    else
        local slot6 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/placeholder.png", effect.get_localised_string("mod_xyy_character_browser_unknown"), "unknown", false, false)
        UIComponent_move_relative(slot6, xyy_select_character_panel, xoffset+180*6, yoffset, false)
    end
    
    if xyy_character_lottery_pool[4] then
        local slot7 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/".. xyy_character_lottery_pool[4] ..".png", effect.get_localised_string("mod_xyy_character_browser_".. xyy_character_lottery_pool[4]), xyy_character_lottery_pool[4], false, true)
        UIComponent_move_relative(slot7, xyy_select_character_panel, xoffset+180*7, yoffset, false)
    else
        local slot7 = add_character_pool(xyy_select_character_panel, 160, "ui/skins/default/character_browser/placeholder.png", effect.get_localised_string("mod_xyy_character_browser_unknown"), "unknown", false, false)
        UIComponent_move_relative(slot7, xyy_select_character_panel, xoffset+180*7, yoffset, false)
    end
