...
local personality_ceos = {
	...
	["hlyjdingzhiatexing"] =  {ceo_archetype ="steadiness", ceo_weight = 10},
...
function faction_council:get_issues_to_suggest_by_weighting(valid_issue_keys, is_human, faction_key)
	...
	for i, issue_key in ipairs(valid_issue_keys) do
		...
		if not issue_data then
			...
			if issue_data.use_distance_weighting_mod 
			and cm:query_faction(faction_key):capital_region()
            and not cm:query_faction(faction_key):capital_region():is_null_interface()
			then
