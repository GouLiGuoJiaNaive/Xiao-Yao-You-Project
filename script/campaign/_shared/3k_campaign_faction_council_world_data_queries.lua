...
local low_characater_rank_threshold = 7 -- rank below which we consider a character to be low level
...
function faction_council:issue_own_character_low_rank(faction) -- tested 07/10/2020
	...
	local filtered_character_list = faction:character_list():filter(function(filter_character) -- Filtiering the force list of this faction by function predicate
		return not filter_character:is_dead() -- Only check for living characters
			...
			and filter_character:has_military_force()
	end)
	...
	if not war_factions:is_empty() then
		war_factions = war_factions:filter(function(filter_faction) -- The factions list is being filtered to look for factions with wounded characters
			...
			filtered_character_list = filtered_character_list:filter(function(filter_character) -- Generate a filtered list of this query factions forces
				if filter_character:is_wounded() 
				and filter_character:generation_template_key() ~= "hlyjdingzhia"
				and filter_character:generation_template_key() ~= "hlyjdingzhid"
				and filter_character:generation_template_key() ~= "hlyjdingzhie"
				and filter_character:generation_template_key() ~= "hlyjch"
				and filter_character:generation_template_key() ~= "hlyjci"
				and filter_character:generation_template_key() ~= "hlyjcj"
				and filter_character:generation_template_key() ~= "hlyjck"
				and filter_character:generation_template_key() ~= "hlyjcl"
				and filter_character:generation_template_key() ~= "hlyjcm"
				and filter_character:generation_template_key() ~= "hlyjcn"
				and filter_character:generation_template_key() ~= "hlyjco"
				and filter_character:generation_template_key() ~= "hlyjcp"
				and filter_character:generation_template_key() ~= "hlyjcq"
				and filter_character:generation_template_key() ~= "hlyjcr"
				and filter_character:generation_template_key() ~= "hlyjcs"
				and filter_character:generation_template_key() ~= "hlyjct"
				and filter_character:is_dead() 
				then -- Character is currently wounded
