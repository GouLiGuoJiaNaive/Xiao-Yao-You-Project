	...
	local trigger_datas = {
		...
		round_start = { 
			...
			triggers = {
				...
				{ trigger_key = "3k_ceo_trigger_traits_round_start_metal", element = traits.ATTRIBUTES.ATT_METAL, num_ceos = 6 },
...
-- BATTLE TRAITS
    core:add_listener(
		...
		function(context)
			self:print("3k_campaign_traits.lua: CampaignBattleLoggedEvent", true );
	...
	core:add_listener(
		...
		function(context) -- What to do if listener fires.
			self:print("3k_campaign_traits.lua: CharacterCaptiveOptionApplied", true );
	...
	core:add_listener(
		...
		function(context) -- What to do if listener fires.
			self:print("3k_campaign_traits.lua: CharacterPostBattleEnslave", true );
	...
	core:add_listener(
		...
		function(context) -- What to do if listener fires.
			self:print("3k_campaign_traits.lua: CharacterPostBattleRelease", true );
	...
	core:add_listener(
		...
		function(context) -- What to do if listener fires.
			self:print("3k_campaign_traits.lua: CharacterPostBattleSlaughter", true );
...
-- DOMESTIC TRAITS
	...
	core:add_listener(
		...
		function(context) -- What to do if listener fires.
			--self:print("3k_campaign_traits.lua: CharacterTurnStart", true );
    ...
    core:add_listener(
        ...
        function(context)
			self:print("3k_campaign_traits.lua: CharacterBuildingCompleted", true );
	...
	core:add_listener(
		...
		function(context)
			self:print("3k_campaign_traits.lua: CharacterWoundReceivedEvent", true );
...
function traits:setup_lovestruck_listeners()
    core:add_listener(
        ...
        function(character_rank_event)
            self:print("3k_campaign_traits.lua: Granting lovestruck trait", true );
...
--[[ traits:setup_legendary_listeners()
	A single shot trigger on campaign start to make sure legendaries are up to date.
]]--
function traits:setup_new_game_legendary_traits()
	...
	for i = 0, faction_list:num_items() - 1 do
		...
		if not faction:is_dead() then
			...
			for j = 0, character_list:num_items() - 1 do
				...
				if self:can_have_legendary_trait(character) then
					self:update_character_legendary_traits(character);
				end;
			end;
		end;
...
function traits:setup_legendary_listeners()
	-- Whenever a character starts their turn check if they became legendary.
	core:add_listener(
		"traits_legendary", -- Unique handle
		"CharacterTurnStart", -- Campaign Event to listen for
		function(context) -- Criteria
			return self:can_have_legendary_trait(context:query_character());
		end,
		function(context) -- What to do if listener fires.
			local query_character = context:query_character();
			self:update_character_legendary_traits(query_character);
		end,
		true --Is persistent
	);
end;

function traits:can_have_legendary_trait(query_character)
...
-- Checks a character's attributes, granting them a legendary trait if needed, and removing it if not eligable any more.
function traits:update_character_legendary_traits(query_character)

	local modify_character_ceos = cm:modify_character(query_character):ceo_management();
	...
	if value >= 100 and not has_legendary_trait then
		...
		local rnd = cm:modify_model():random_number(0, 3);
		...
		self:print( "traits.lua: Legendary Traits: Character gained legendary trait: " .. tostring( query_character:generation_template_key() ) .. " Attribute, Value:" .. tostring(key) .. ":" .. tostring(value), true );
	...
	elseif value < 100 and has_legendary_trait then
		...
		self:print( "traits.lua: Legendary Traits: Character lost legendary trait: " .. tostring( query_character:generation_template_key() ), true );
...
function traits:setup_debug_listeners()
	...
	core:add_cli_listener( "traits.give_trait", 
        function(character_cqi, trait_key)
            self:print("traits.give_trait", false);
    ...
    core:add_cli_listener( "traits.give_physical_trait", 
        function(character_cqi)
            ...
            if query_character and not query_character:is_null_interface() and modify_character and not modify_character:is_null_interface() then
                ...
                self:print("traits.give_physical_trait: rolled " .. random_pct .. "/" .. self.physical_chance_of_serious_wound, true);
                -- Different wounds based on chance.
                if random_pct <= self.physical_chance_of_serious_wound then
                else
                end;
    ...
    core:add_cli_listener( "traits.give_lovestruck_trait", 
        function(character_cqi)
            ...
            if query_character and not query_character:is_null_interface() and modify_character and not modify_character:is_null_interface() then
                self:print("traits.give_physical_trait: fired trigger:" .. self.trigger_key_lovestruck, true);
...
function traits:on_building_constructed( query_character, modify_character )
	self:print("Building completed event fired. No events!")
end;
